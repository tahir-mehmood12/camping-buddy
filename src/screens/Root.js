...
const Root = (props) => {
  ...
  useEffect(() => {
    ...
    if (groups.length > 0) {
      ...
      const currentGroup = UserCtx.current
  ...
  useEffect(() => {
    if (UserCtx.current && groups && UserCtx.current === "new") {
      ...
      UserCtx.current &&
      groups &&
      UserCtx.current !== group?.id &&
      UserCtx.current !== "delete"
    ) {
      ...
      if (groups.length > 0) {
        ...
        console.log(
          "current group has been deleted, switch to the next group",
          groups[0].id
        );
        ...
        console.log(
          "current group has been deleted, no other groups available, add a new group"
        );
        addNewGroup(user, UserCtx.settings?.name);
      }
    }
  ...
  useEffect(() => {
    ...
    if (group.festival.id) {
      ...
      unsubscribe3 = onSnapshot(
        doc(db, "festivals", group.festival.id),
        (doc) => {
          if (doc.data()) {
  ...
  return (
    <div className="container bg">
      ...
      <div className="content">
        ...
        {currentScreen === "list" && <Checklist group={group} />}
        {currentScreen === "notifications" && (
          <Notifications settings={settings} group={group} />
        )}
        {currentScreen === "settings" && (
          <Settings group={group} settings={settings} />
        )}
      </div>
